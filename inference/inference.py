...
# load the model
model = keras.models.load_model('../input/modelhere/pnue_model.h5')

# strings
label = {
    0 : 'Patient has Normal report.',
...
def predict (img_path) :
    ...
    for types in ['NORMAL/', 'PNEUMONIA/'] :
        for path in os.listdir(img_path + types) :
            image = load_img(img_path + types + path, color_mode = 'grayscale', target_size = (128,128))
